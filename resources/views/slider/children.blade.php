@foreach($childrens as $children)
    @if ($page == 'add')
            <option value="{{ $children->id }}" {{ ( $children->id == old('categorie_id')) ? 'selected' : '' }}>
                @for ($j = 0; $j < $nombreIteration; $j++)
                    --
                @endfor
                {{ $children->titre }}
            </option>
            @if(count($children->categories))
                @php ($nombreIteration++)
                @include('categorie.children',[
                    'childrens' => $children->categories,
            ...
            @endif
    @elseif ($page == 'edit')
        <option value="{{ $children->id }}" {{ ($children->id == $category->categorie_id) ? 'selected' : '' }}>
            @for ($j = 0; $j < $nombreIteration; $j++)
                --
            @endfor
            {{ $children->titre }}
    ...
    @elseif($page == 'index')
        ...
        <tr>
            ...
            <td>
                <a href="{{ url()->current().'/'.$children->id.'/edit' }}" class="btn btn-success waves-effect" data-toggle="tooltip" data-placement="top" title="Modifier" data-original-title="Modifier"><i class="icofont icofont-edit"></i>
                </a>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form action="{{ url()->current().'/'.$children->id }}" method="post">
                                    @method('DELETE')
                                    @csrf
        ...
        {{-- @php(dd($children->categories->toArray())) --}}
        @if(count($children->categories))
            @php ($nombreIteration++)
            @include('categorie.children',[
                'childrens' => $children->categories,
            ...
            @php ($nombreIteration--)
        @endif
        @php ($i += count($children->categories))
    @elseif($page == 'post')
        <tr>
            <td>{{ $i }}</td>
            <td>
